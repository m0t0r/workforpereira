...
- **ADR-0017's `test` task inherits this linter.** Tests are colocated as `src/**/*.test.ts` and in
  scope for lint, so whoever wires up Vitest should enable oxlint's `vitest` plugin in the baseline
  at the same time — the rules that catch a forgotten `await` on `expect` are exactly the ones the
  type-aware set already argues for. The pull-request gate ADR-0017 names,
  `turbo run lint check-types test db:check`, needs no change: `lint` is the same task, now oxlint.
