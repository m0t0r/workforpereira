...
| Every request is a tracked case with a legal clock               | L.1581 arts. 14, 15                             | A `data_request` entity: type (`consulta` \| `reclamo` \| `revocacion` \| `supresion`), `received_at`, computed `due_at` in **días hábiles on the Colombian calendar** (`America/Bogota`, incl. _festivos_ — **19 since Ley 2578 de 1 junio 2026 added 9 July**, eleven of them movable to the following Monday under Ley 51 de 1983), `extension_notified_at`, `resolved_at`. Deadlines must alarm before they lapse.                                                                  |
| The `reclamo en trámite` legend                                  | L.1581 art. 15.2, art. 18(g)                    | A marker on the data carrying the motive, applied within 2 días hábiles and kept until the reclamo is decided. ~~A **persistent state flag on the affected records**~~ — **corrected by ADR-0020**: derived from the open reclamo rather than stored, so the 2-day term cannot be missed. ~~and, per art. 4(f), the flagged data should not be circulating to employers meanwhile: art. 18(i) forbids circulating data being contested~~ — **this is wrong, see §4's correction note.** |
...
### Correction (ADR-0020): art. 18(i) does not forbid circulating contested data

This section, and checklist item 20 in §11, both read art. 18(i) as a standing prohibition. It is
not. Verbatim from the consolidated text:
...
It is **conjunctive** — contested _and_ blocked by SIC order — and it is a duty of the **Encargado**.
Art. 17, which lists the _Responsable_'s duties, has no equivalent; its (l) requires only
_"Informar al Encargado del Tratamiento cuando determinada información se encuentra en discusión por
parte del Titular, una vez se haya presentado la reclamación y no haya finalizado el trámite
respectivo"_.

**So a reclamo suppresses nothing.** The art. 15.2 legend is a statement about the data, not a
restriction on it. ADR-0020 rejects the suppression this section assumed, on the ground that hiding a
Person's Publications while they contest one field would remove the income surface the platform
exists to provide — for up to 23 business days, on a misreading.

**Also missed here: art. 18(h) is a second legend** — _"información en discusión judicial"_, inserted
once a competent authority notifies us of judicial proceedings about data quality. ADR-0020 defers it
with reasons rather than leaving it unrecorded.
...
| 20  | ~~Do not circulate data being contested~~                                                                      | ~~L.1581 art. 18(i)~~                                                         | **Struck by ADR-0020 — this obligation does not exist as stated.** See §4's correction note.         |
