...
function FieldError({
  ...
  const content = useMemo(() => {
    ...
    // **Edited from the registry output.** shadcn ships `uniqueErrors?.length == 1` here, and
    // `uniqueErrors` is a spread of `Map.values()` — an array, never nullish — so the optional chain
    // is `typescript(no-unnecessary-condition)` under this repo's `--type-aware` lint (ADR-0018) and
    // fails `pnpm lint`. Re-running `shadcn add field --overwrite` reintroduces it.
